...
This document covers everything specific to the Flare network: FTSO v2 on-chain price feeds, native FLR payments via the x402 protocol, USDT0 EIP-3009 payments, and the proactive monitoring loop.
...
3. [USDT0 x402 Payments on Flare](#3-usdt0-x402-payments-on-flare)
4. [Proactive Monitoring Loop](#4-proactive-monitoring-loop)
...
## 3. USDT0 x402 Payments on Flare
...
| `X402_ENABLE_FLARE_USDT0` | `true` | Enable USDT0 EIP-3009 payment path |
| `X402_FLARE_CHAIN_ID` | `14` | Flare mainnet chain ID |
| `X402_USDT0_FLARE_ADDRESS` | `0xe7cd86e13AC4309349F30B3435a9d337750fC82D` | USDT0 token contract |
| `X402_USDT0_DECIMALS` | `6` | USDT0 decimal places |
| `X402_FLARE_FACILITATOR_ADDRESS` | — | Deployed X402Facilitator address |
| `X402_USDT0_RECIPIENT` | — | USDT0 payment recipient |
| `X402_USDT0_AMOUNT` | `0.001` | Default USDT0 price |
| `X402_FLARE_CONFIRMATIONS` | `1` | Confirmations before settlement is accepted |
| `X402_SETTLEMENT_MODE` | `client` | `client` or `server` settlement mode |
| `X402_SETTLER_PRIVATE_KEY` | — | Required only for server-settlement mode |
...
```
                        ...
                        │  └─► settlePayment(USDT0 EIP-3009) │
                        │                                    │
                        └────────────────────────────────────┘
                               ▲                ▲
                    FTSO read  │    anchor tx   │  FLR/USDT0 payment tx
                               │                │
                   ┌───────────────────────────────────────────────┐
