...
## USDT0 on Flare — EIP-3009 Payment Path (`eip3009_facilitator`)

In addition to USDC on Base, the middleware supports **USDT0 on Flare** via the EIP-3009 `transferWithAuthorization` mechanism.
...
## Native FLR on Flare — Native Transfer Payment Path (`native_transfer`)
...
6. **USDT0 signing type**: Always sign `TransferWithAuthorization` EIP-712 typed data (not `ReceiveWithAuthorization`) — the facilitator calls `transferWithAuthorization` internally, so using the wrong typehash causes on-chain signature verification to fail.
